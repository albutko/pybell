...
def load_calibration_coefficients(file_path):
    ...
    k_shape = calib.get('K').shape
    d_shape = calib.get('D').shape
    # Variables are the correct shape
    assert k_shape == (3, 3), f'Calibration Matrix is wrong shape {k_shape} != (3,3)'
    assert d_shape == (1, 4), f'Distortion coefficients is wrong shape {d_shape} != (1,4)'
